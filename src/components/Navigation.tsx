...
export default function Navigation() {
  ...
  // ตรวจสอบการบริจาคที่รออนุมัติ
  const [myPendingDonations, setMyPendingDonations] = React.useState<any[]>([]);
  React.useEffect(() => {
    if (!user) return;
    const donatesRef = ref(db, 'guilddonate');
    const cashRef = ref(db, 'guilddonatecash');
    ...
    const unsubDonates = onValue(donatesRef, (snapshot) => {
      if (snapshot.exists()) {
        const data = snapshot.val();
        const myWaiting = Object.values(data).filter((d: any) => 
          d.userId === user.uid && d.status === 'waiting'
        );
        setMyPendingDonations(myWaiting);
      }
    });
    ...
    const unsubCash = onValue(cashRef, (snapshot) => {
      if (snapshot.exists()) {
        const data = snapshot.val();
        const myWaitingCash = Object.values(data).filter((d: any) => 
          d.userId === user.uid && d.status === 'waiting' && d.type === 'cash'
        );
        setMyPendingDonations(prev => [...prev, ...myWaitingCash]);
      }
    });
    ...
    <>
      ...
      {myPendingDonations.length > 0 ? (
        <div className="hidden lg:flex w-full justify-center z-[9999] fixed top-14 left-0 pointer-events-none">
          <div className="flex items-center gap-2 px-3 py-1.5 mt-1 rounded-lg shadow border border-yellow-200 bg-gradient-to-r from-yellow-100 via-orange-50 to-yellow-100 animate-fade-in pointer-events-auto min-w-[280px] max-w-[450px]">
            <span className="text-lg">⏳</span>
            <span className="text-yellow-800 font-semibold text-xs">
              คุณมี <b className="font-bold">{myPendingDonations.length}</b> รายการบริจาคที่รอการอนุมัติ
            </span>
            <a
              href="https://discord.com/users/1163943838826631258"
              target="_blank"
              rel="noopener noreferrer"
              className="ml-1 px-2 py-0.5 rounded bg-gradient-to-r from-[#5865F2] to-[#4752C4] text-white font-bold text-xs shadow flex items-center gap-1 transition transform hover:scale-105 focus:scale-105 animate-pulse hover:animate-none focus:animate-none"
              style={{ boxShadow: '0 0 8px #5865F2, 0 0 2px #4752C4' }}
            >
